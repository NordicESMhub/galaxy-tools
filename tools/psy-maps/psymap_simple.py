...
#
#
# usage: psymap_simple.py [-h] [--proj PROJ] [--cmap CMAP] [--output OUTPUT] [-v]
#               input varname
#
...
class PsyPlot ():
  ...
  def plot(self):
      if self.cmap is None and self.proj is None:
         maps = psy.plot.mapplot(self.input, name=self.varname, clabel='{desc}')
      elif self.proj is None:
         maps = psy.plot.mapplot(self.input, name=self.varname, cmap=self.cmap,
               clabel='{desc}') 
      elif self.cmap is None:
         maps = psy.plot.mapplot(self.input, name=self.varname, 
               projection = self.proj, clabel='{desc}') 
      else:
         maps = psy.plot.mapplot(self.input, name=self.varname, cmap=self.cmap,
               projection = self.proj, clabel='{desc}') 
...
if __name__ == '__main__':
  ...
  psymap_plot(args.input, args.proj, args.varname, args.cmap, args.output, args.verbose)
