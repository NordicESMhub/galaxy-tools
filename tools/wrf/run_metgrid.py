...
def main():
    usage = "usage: %prog --expid expid " \
            "--tbl METGRID.TBL [--version=wrf_version]"
    ...
    print(metgrid)
    check_metgrid = (os.path.basename(metgrid) == "METGRID.TBL")
    check_metgrid_dir = (os.path.dirname(metgrid) == exp_rundir + '/metgrid')
    if check_metgrid and check_metgrid_dir:
