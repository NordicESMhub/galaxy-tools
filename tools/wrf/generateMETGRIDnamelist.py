...
def main():
    usage = "usage: %prog --expid=<WRFexpid> --metgrid=<metgrid_script> " \
            "[--output=namelist.wps]"
    parser = OptionParser(usage=usage)

    parser.add_option("-o", "--output", dest="metgrid_namelist",
                      help="namelist.wps file used by WPS (run metgrid.exe)",
                      metavar="metgrid_namelist")

    parser.add_option("-w", "--expid", dest="expid",
                      help="WRF experiment setup file", metavar="expid")

    parser.add_option("-m", "--metgrid", dest="metgrid",
                      help="metgrid script to generate metgrid namelist",
                      metavar="metgrid")
        ...
        for line in nmlfile:
            if 'interval_seconds' in line:
                ...
                if (interval_seconds != newinterval_seconds):
                    print("Error: interval_seconds should be identical ",
                          "among the datasets!", interval_seconds,
                          newinterval_seconds)
        nmlfile.close()
    ...
    for line in geogrid_namelist:
        if '&share' in line:
            ...
            new_file.write('interval_seconds = ' +
...
# geogrid file geo_em*
    ...
    for path in datapath:
        for dirName, subdirList, fileList in os.walk(path.strip() +
                                                     '/' +
                                                     options.expid.strip() +
                                                     '/'):
            for fname in fileList:
