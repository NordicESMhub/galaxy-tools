...
def main():
    usage = "usage: %prog --expid expid [--namelist namelist_input] " \
            "[--case wrf_case] [--d physics_data_dir] [--npes nprocs]"
    ...
    parser.add_option("-n", "--namelist", dest="namelist_input",
                      help="namelist.input file for running WRF",
                      metavar="namelist.input")

    parser.add_option("-c", "--case", dest="wrf_case",
                      help="type of experiment (em_real for real cases)",
                      metavar="wrf_case")

    parser.add_option("-d", "--data_dir", dest="physics_data_dir",
                      help="directory where all model physics data " +
                           "files (*.TBL, *_DATA, *.formatted, tr*t*) " +
                           "can be found for running erf.exe",
                      metavar="physics_data_dir")

    parser.add_option("-p", "--npes", dest="npes",
                      help="number of MPI tasks for running WRF",
                      metavar="npes")
    ...
    check_namelist1 = (not os.path.isfile(namelist_input))
    check_namelist2 = (options.namelist_input != namelist_input)
    check_namelist = (check_namelist1 or check_namelist2)
    if options.namelist_input and check_namelist:
        shutil.copy2(options.namelist_input, exp_rundir + '/namelist.input')

    if not os.path.isfile(namelist_input):
        parser.error("the file namelist.input does not exist in " +
                     namelist_input)
        os.chdir(exp_rundir)
    ...
    # set-up *.TBL files (GENPARM, LANDUSE, SOILPARM, URBPARM,
    # VEGPARM, MPTABLE) in exp_rundir
    ...
    if os.path.isfile(wrf_path + '/wrf.exe'):
        if int(npes) > 1:
            command = 'module load wrf; ulimit -s unlimited; mpirun ' + \
                      wrf_path + '/wrf.exe'
        else:
            command = 'module load wrf; ulimit -s unlimited; ' + \
                      wrf_path + '/wrf.exe'
