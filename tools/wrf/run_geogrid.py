		...
		if 'geog_data_path' in line or 'GEOG_DATA_PATH' in line:
			...
			print newline1
		        ...
		        new_file.write(newline2)
                        ...
                        print "Removed user-defined opt_geogrid_tbl_path; file copied in experiment directory"
		else:
		        new_file.write(line)
...
# tblname file present in exp_rundir               
        ...
        if os.path.basename(geogrid_tbl) == "GEOGRID.TBL" and os.path.dirname(geogrid_tbl) == exp_rundir:
...
def main():
	usage = "usage: %prog --path namelist.wps_directory --expid experiment_identifier [--version=wrf_version] [--wps_geog WPS_GEOG] [--tbl GEOGRID.TBL]"
	parser = OptionParser(usage=usage)
	parser.add_option("-p", "--path", dest="path",
                  help="location of namelist.wps", metavar="namelist.wps")
	parser.add_option("-w", "--wps_geog", dest="wps_geog",
                  help="location of terrestrial input data", metavar="WPS_GEOG")
	parser.add_option("-i", "--expid", dest="expid",
                  help="Experiment identifier", metavar="expid")
	parser.add_option("-t", "--tbl", dest="geogrid_tbl",
                  help="GEOGRID.TBL file for running geogrid.exe", metavar="geogrid_tbl")
	...
	if not os.path.isfile(namelist_wps):
		parser.error("the file namelist.wps does not exist in "+ namelist_wps)
