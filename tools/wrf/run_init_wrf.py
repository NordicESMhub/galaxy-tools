...
def main():
    ...
    parser.add_option("-c", "--case", dest="wrf_case",
                      help="type of experiment (em_real for data real cases)",
                      metavar="wrf_case")
    msg = "directory where model physics data files" \
          "(*.TBL, *_DATA, *.formatted, tr*t*) can be " \
          "found for running wrf.exe"
    ...
    not_cond = (not os.path.isfile(namelist_input))
    not_cond = (not_cond or options.namelist_input != namelist_input)
    copy_condition = options.namelist_input and not_cond
    if copy_condition:
