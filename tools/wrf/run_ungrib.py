...
# vtable file present in exp_rundir
	...
	if os.path.basename(vtable) == "Vtable" and os.path.dirname(vtable) == exp_rundir:
...
def main():
	usage = "usage: %prog --expid expid --vtable vtable --datadir datadir [--version wrf_version] [--start_date startdate --end_date enddate] [--interval_seconds val] [--prefix FILE]"
	parser = OptionParser(usage=usage)
	parser.add_option("-s", "--start_date", dest="start_date",
                  help=" A list of MAX_DOM character strings of the form 'YYYY-MM-DD_HH:mm:ss' specifying the starting UTC date of the simulation for each domain. ", metavar="startdate")
	parser.add_option("-e", "--end_date", dest="end_date",
                  help="A list of MAX_DOM character strings of the form 'YYYY-MM-DD_HH:mm:ss' specifying the ending UTC date of the simulation for each domain", metavar="enddate")
	parser.add_option("-t", "--interval_seconds", dest="interval_seconds",
                  help="The integer number of seconds between time-varying meteorological input files. No default value.", metavar="interval_seconds")

	parser.add_option("-p", "--prefix", dest="prefix",
                  help="prefix of the intermediate meteorological data files", metavar="FILE")
	...
	parser.add_option("-d", "--datadir", dest="datadir",
                  help="Directory where input fields can be found", metavar="datadir")
...
# load default WRF version
        ...
        print dirroot
        if not options.datadir or ( not os.path.exists(options.datadir) and not os.path.exists(dirroot)) :
...
# set parameters in namelist.wps if required
	set_wps_parameters(namelist_wps,options.start_date, options.end_date, options.interval_seconds, options.prefix)
